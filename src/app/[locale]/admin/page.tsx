...
export default async function AdminPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8 px-4 py-6">
      <div>
        <h1 className="text-xl font-semibold text-brand-950">{dict.admin.title}</h1>
        <p className="text-sm text-slate-500">{dict.admin.subtitle}</p>
      </div>
      ...
      <section className="space-y-3">
        ...
        {openReports.length === 0 ? (
          <p className="text-sm text-slate-500">{dict.admin.noReports}</p>
        ) : (
          openReports.map((report) => (
            <Card key={report.id} className="p-3">
              ...
              {report.details && <p className="mt-1 text-xs text-slate-600">{report.details}</p>}
      ...
      <section className="space-y-3">
        ...
        {pendingProviders.length === 0 ? (
          <p className="text-sm text-slate-500">{dict.admin.noVerification}</p>
        ) : (
          pendingProviders.map((provider) => (
            <Card key={provider.id} className="p-3">
              ...
              {provider.description && <p className="mt-1 line-clamp-3 text-xs text-slate-600">{provider.description}</p>}
      ...
      <section className="space-y-3">
        ...
        {pendingContributions.length === 0 ? (
          <p className="text-sm text-slate-500">{dict.admin.noContributions}</p>
        ) : (
          pendingContributions.map((contribution) => (
            <Card key={contribution.id} className="p-3">
              <p className="text-sm font-semibold text-brand-950">{contribution.type}</p>
              <pre className="mt-1 max-h-32 overflow-auto rounded-lg bg-slate-50 p-2 text-[11px] leading-relaxed text-slate-600">
