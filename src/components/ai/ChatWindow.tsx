  ...
  return (
    <div className="mx-auto flex h-[calc(100dvh-136px)] max-w-2xl flex-col px-4 py-4 md:h-[calc(100dvh-64px)]">
      ...
      >
            ...
            <div className="flex flex-col gap-2">
              {suggestions.map((s) => (
                <button
                  ...
                  className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-start text-sm text-brand-800 hover:bg-slate-50"
                >
                  {s}
                </button>
              ))}
            ...
            <div
              className={
                m.role === "user"
                  ...
                  : "max-w-[80%] rounded-2xl rounded-bl-sm bg-white px-4 py-2.5 text-sm text-brand-950 shadow-[var(--shadow-card)]"
              }
            >
              {m.content}
            </div>
        ...
        {sending && <p className="text-xs text-slate-400">{dict.ai.thinking}</p>}
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 border-t border-brand-900/8 pt-3">
